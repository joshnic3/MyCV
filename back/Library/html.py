...
from Library.constants import CvParts, SCHEMA, Mapping
...
class HTMLModals:
    ...
    def _save_delete_buttons(edit_type, row_id):
        ...
        delete = '' if edit_type not in Mapping.CAN_DELETE else HTMLComponents.button(
            'Delete {}'.format(Mapping.SECTION_TITLE.get(edit_type)),
    ...
    @staticmethod
    def _edit_form(edit_type, row_id, user_data=None):

        def get_current_row():
            if user_data is not None:
                data_rows = user_data if edit_type == CvParts.INFO else user_data.get(edit_type)
        ...
        # Load data maps.
        date_parts = [Mapping.START_DATE, Mapping.END_DATE]
        text_area_parts = [Mapping.CONTENT]
        schema_map = Mapping.SCHEMA.get(edit_type)
        display_name_map = Mapping.DISPLAY_NAME.get(edit_type)
        ...
        for key in Mapping.KEYS:
            if schema_map.get(key) is not None:
                form_parts.append('<h5 class="">{}</h5>'.format(display_name_map.get(key)))
                current_value = current_data.get(schema_map.get(key)) if current_data else ''
                if key in date_parts:
                    form_parts.append(HTMLComponents.date(key, 'form-control', value=current_value, max_today=True))
                elif key in text_area_parts:
        ...
        if user_data is not None:
            form_parts.append(HTMLModals._save_delete_buttons(edit_type, row_id))
        else:
            form_parts.append(HTMLModals._add_button(edit_type, row_id))
    ...
    @staticmethod
    def share_modal(base_url, user_id):
        # Generate user link.
    ...
    def manage_modal(user_data):
        ...
        form_parts = [
            ...
            HTMLComponents.alert(HTMLModals.SUCCESS_INJECTION_ID, '', alert_type='success'),
            '<h5 class="">Email</h5>',
            HTMLComponents.input('manageEmail', 'text', 'form-control', value=user_data[SCHEMA.USER_EMAIL]),

            HTMLComponents.button('Save', classes='btn btn-primary btn-sm form-control',
                                  onclick='editEmail()'),
            '<hr>',
            '<h5 class="">Change Password</h5>',
            HTMLComponents.input('manageNewPassword', 'password', 'form-control', placeholder='New Password'),
            HTMLComponents.input('manageConfirmNewPassword', 'password', 'form-control', placeholder='Confirm New Password'),
            ...
            HTMLComponents.button('Close and Delete All Data', classes='btn btn-outline-danger btn-sm form-control',
                                  onclick='requestDeleteModal(\'{}\', \'{}\')'.format(CvParts.INFO,
                                                                                      user_data[SCHEMA.ID]))
        ]
        form_html = html.get('form').format(classes='p-1 m-0"')
        form = HTMLComponents.wrap(form_html, ''.join(form_parts))
    ...
    @staticmethod
    def delete_modal(delete_type, row_id):
        # Generate HTML.
        html = HTMLComponents.HTML.get('input')
    ...
    @staticmethod
    def edit_modal(edit_type, row_id, user_data):
        title = Mapping.SECTION_TITLE.get(edit_type)
        modal_parts = [
            HTMLComponents.alert(HTMLModals.ALERT_INJECTION_ID, '', alert_type='warning'),
            HTMLComponents.alert(HTMLModals.SUCCESS_INJECTION_ID, '', alert_type='success'),
            HTMLModals._edit_form(edit_type, row_id, user_data=user_data)
        ]
        return HTMLComponents.modal(HTMLModals.INJECTION_DIV_ID, 'Edit ' + (title if title else ''), ''.join(modal_parts))

    @staticmethod
    def add_modal(add_type, row_id):
        form = HTMLModals._edit_form(add_type, row_id)
        title = Mapping.SECTION_TITLE.get(add_type)
